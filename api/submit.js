...
if (dp) {
  ...
  const sym  = PLANET_SYMBOL[planetName] || '';
  ...
  // Simbolo do planeta (maior)
  if (sym) page.drawText(sym, {
    x: px0 + 4, y: pillY + 5, size: 13,
    font: dejaVuFont, color: temDados ? branco : textMedio,
  });
  // Portao.Linha (menor, depois do simbolo)
  if (gate) page.drawText(gate, {
    x: px0 + 20, y: pillY + 6, size: 9,
    font: dejaVuFont, color: temDados ? branco : textMedio,
  });
}

 ...
 * Fluxo: resolve timezone → gera HD+SVG → converte PDF → envia 2 e-mails
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // waitUntil garante que a Vercel mantém a função viva até o PDF ser enviado
    waitUntil((async () => {
      try {
        console.log(`[HD] Iniciando geração para ${email}`);
        const timezone  = await resolveTimezone(local);
        console.log(`[HD] Timezone: ${timezone}`);
        ...
        console.log(`[HD] SVG recebido (${svgString.length} chars)`);

        const pdfBytes  = await svgToPdf(svgString, nome, hdData);
        const pdfBase64 = Buffer.from(pdfBytes).toString('base64');
        console.log(`[HD] PDF gerado (${pdfBytes.length} bytes)`);
        ...
        console.log(`[HD] PDF enviado com sucesso para ${email}`);
      } catch (err) {
        console.error(`[HD] Erro no background: ${err.message}`);
      }
    })());

...
async function svgToPdf(svgString, nome, hdData) {
  ...
  const pngBuffer = await sharp(Buffer.from(svgString), { limitInputPixels: false })
    .resize({ width: 800 })
    .png()
    .toBuffer();
  ...
  page.drawRectangle({ x: 0, y: 0, width: 841.89, height: 595.28, color: rgb(0.102, 0.078, 0.063) });
  ...
  page.drawImage(pngImage, { x: 20, y: (595.28 - imgDims.height) / 2, width: imgDims.width, height: imgDims.height });
  ...
  rows.forEach(([label, value], i) => {
    const y = 506 - i * 42;
    page.drawRectangle({ x: panelX - 4, y: y - 8, width: 329, height: 36, color: rgb(0.173, 0.133, 0.110), borderColor: coffee, borderWidth: 0.3, borderOpacity: 0.4 });
    page.drawText(label,         { x: panelX + 4, y: y + 14, size: 7, color: peach });
    page.drawText(String(value), { x: panelX + 4, y: y + 2,  size: 9, color: cream });
  });

  page.drawText('© 2025 Vida Autoral · Todos os direitos reservados', { x: panelX, y: 16, size: 7, color: rgb(0.4, 0.35, 0.3) });
...
<style>body{margin:0;padding:0;background:#E9D7C0;font-family:Georgia,serif;}.wrap{max-width:580px;margin:0 auto;background:#fff;}.header{background:#1a1410;padding:2.5rem 2rem;text-align:center;}.header h1{color:#E9D7C0;font-size:1.3rem;font-weight:400;letter-spacing:.15em;margin:.8rem 0 0;}.body{padding:2.5rem 2rem;}p{color:#4a3a2e;font-size:.95rem;line-height:1.8;margin-bottom:1rem;}.info{background:#FED8A6;border-left:3px solid #9B7D61;padding:1rem 1.2rem;margin:1.5rem 0;}.info p{margin:0;font-size:.88rem;color:#5a3f28;line-height:1.8;}.footer{background:#1a1410;padding:1.5rem 2rem;text-align:center;}.footer p{color:rgba(233,215,192,.35);font-size:.7rem;margin:0;}</style>
  ...
  <div class="body">
    ...
    <p>O gráfico revela seu Tipo Energético, Autoridade, Perfil, Centros e muito mais. Qualquer dúvida, responda este e-mail.</p>
    <p style="font-size:.85rem;color:#9b836f;">Com carinho,<br/><strong>Equipe Vida Autoral</strong></p>
  </div>
  <div class="footer"><p>© 2025 Vida Autoral · Todos os direitos reservados</p></div>
</div></body></html>`;
...
// ── Handler — faz tudo sequencialmente antes de responder ────────────────────
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`[1] Confirmação enviada para ${email}`);

    // 2. Gera o mapa
    const timezone = await resolveTimezone(local);
    console.log(`[2] Timezone: ${timezone}`);

    const hdData = await generateHDChart(data, hora, timezone);
    const svgString = hdData.SVG;
    if (!svgString) throw new Error('SVG não retornado pela API do Bodygraph');
    console.log(`[3] SVG recebido (${svgString.length} chars)`);

    // 3. Gera PDF
    const pdfBytes  = await svgToPdf(svgString, nome, hdData);
    const pdfBase64 = Buffer.from(pdfBytes).toString('base64');
    console.log(`[4] PDF gerado (${pdfBytes.length} bytes)`);

    // 4. Envia e-mail com PDF
    await sendEmail({
      to: email,
    ...
    console.log(`[5] PDF enviado com sucesso para ${email}`);

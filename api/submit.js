...
const pillStepFull = AREA_H / PLANET_ORDER.length;
const pillGap = pillStepFull - PILL_H;           // gap original entre pills
const pillStep = PILL_H + pillGap / 2;            // gap reduzido à metade
const PILLS_OFFSET_Y = 8;                         // 8px para baixo
...
// Cabeçalho direito — faixa marrom sem logo nem textos
doc.rect(DIVIDER, 0, W - DIVIDER, 56).fill(COFFEE);

// Nome e dados de nascimento ACIMA da faixa marrom, fonte branca, alinhado à esquerda
// A faixa começa em y=0, então o texto fica sobre ela (a faixa serve de fundo)
const nomeDisplay = nome.length > 28 ? nome.slice(0,28)+'...' : nome;
doc.font('DejaVu').fontSize(13).fillColor('#ffffff')
   .text(nomeDisplay.toUpperCase(), DX, 12, { lineBreak: false });

// Dados de nascimento (local · data · hora) — cor branca levemente opaca
doc.font('DejaVu').fontSize(7).fillColor('#ffffff')
   .text(local + '  \u00b7  ' + data + '  \u00b7  ' + hora, DX, 30, { lineBreak: false });
...
const CARDS_START_Y = 64;   // logo abaixo da faixa marrom (nome/datas estão dentro da faixa)

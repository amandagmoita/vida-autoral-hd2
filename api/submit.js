...
// Substitui fill branco/claro em atributos inline dentro de tags <text ...>
// O resvg ignora CSS !important — só atributos inline funcionam de forma confiável.
function corrigirTextosNoSvg(svg) {
  // Captura cada tag <text ...> e substitui fills brancos por preto
  return svg.replace(/<text([^>]*)>/g, (match, attrs) => {
    let fixed = attrs;
    // fill="white" | fill="#fff" | fill="#ffffff" | fill="#FFF" etc.
    fixed = fixed.replace(/fill="(white|#[fF]{3}|#[fF]{6})"/g, 'fill="#111111"');
    // fill='white' com aspas simples
    fixed = fixed.replace(/fill='(white|#[fF]{3}|#[fF]{6})'/g, "fill='#111111'");
    return `<text${fixed}>`;
  });
}
...
function prepararSvg(svgString) {
  ...
  // Substitui fills brancos em <text> tags diretamente nos atributos inline
  svg = corrigirTextosNoSvg(svg);

  // Substitui cores de fundo muito escuras por branco (fundos dos centros)
  // Cuidado: NÃO substitui em elementos <text>
  svg = svg.replace(/(<(?!text)[a-zA-Z][^>]* fill=)"#1[0-9a-fA-F]{5}"/g, '$1"#ffffff"');
  svg = svg.replace(/(<(?!text)[a-zA-Z][^>]* fill=)"#0[0-9a-fA-F]{5}"/g, '$1"#f5f3f0"');
